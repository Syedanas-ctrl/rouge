    ...
    <Accordion type="single" collapsible className="w-full">
      {Object.values(ComponentTypes).map((component) => (
        <AccordionItem key={component} value={component}>
          ...
          <AccordionContent className={cn(display === "grid" ? "grid grid-cols-3 gap-2 justify-items-stretch" : "flex flex-col gap-2")}>
            {Object.values(ComponentTypes).map((builderComponent, index) => (
              <Child key={builderComponent} >
                List {index + 1}
              </Child>
            ))}
          </AccordionContent>
...
const GridChild = ({ children }: { children: React.ReactNode }) => {
  return <Button variant={"outline"} size={"sm"} className="flex items-center justify-center rounded-md" >{children}</Button>;
};

const FlexChild = ({ children }: { children: React.ReactNode }) => {
  return <Button variant={"ghost"} size={"sm"} className="flex items-center justify-start rounded-md" >
    {children}
    </Button>;
};
